...
const TARGET_RADIUS = 12; // distance to keep from the target
...
const monsters = Array.from({ length: NUM_MONSTERS }, () => {
  ...
  return {
    ...
    update(dt) {
      ...
      // Move with original steering formula (no separation)
      const dx = tx - this.x, dy = ty - this.y;
      const dist = Math.sqrt(dx * dx + dy * dy) || 1;
      const targetVx = (dx / dist) * MONSTER_SPEED_SEC;
      const targetVy = (dy / dist) * MONSTER_SPEED_SEC;
