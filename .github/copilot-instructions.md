...
- **Engine**: Singleton orchestrating the game loop. Coordinates all managers and maintains `npcs` array for update/draw cycles.
...
### Game Objects
Implement `update(deltaTime)` and `draw(ctx, img, cameraManager)` methods. Engine calls `npcs.sort()` by Y before rendering (isometric-style depth sorting via `getSortY()`).
...
- **Zone Loading**: `engine.loadZone(zoneName)` resets `npcs`, must have entry in `ZoneConfig`
...
**Add game object**: Implement update/draw, add to `npcs` array in `Engine.loadZone()`.
